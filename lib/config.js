...
var VERBS = {
	...
	ru: {
		common: ['рассказал(?:а)?', 'сказал(?:а)?', 'заявил(?:а)?', 'отметил(?:а)?', 'добавил(?:а)?', 'рассказал(?:а)?', 'подчеркнул(?:а)?', 'говорит', 'верит', 'считает', 'думает', 'спрашивает', 'напоминает', 'признал(?:а)?', 'ответил(?:а)?', 'заключил(?:а)?', 'заметил(?:а)?', 'написал(?:а)?', 'пояснил(?:а)?']
	},
	bg: {
...
var DATA = {
	...
	ru: {
		quoteMarks: [QUOTE_MARKS[0], QUOTE_MARKS[1], QUOTE_MARKS[2], QUOTE_MARKS[3], QUOTE_MARKS[4]],
			...
			',?' + NAME_SPACE + '(?:' + SEPARATOR + '?[ \\t]+)?' + verbs(VERBS.ru.common) + NAME_SPACE + END_NAME
		],
		start: [
			START_NAME + ':' + NAME_SPACE
		]
	...
	bg: {
		quoteMarks: [QUOTE_MARKS[1], QUOTE_MARKS[2], QUOTE_MARKS[3], QUOTE_MARKS[4]],
			...
			',?' + NAME_SPACE + '(?:' + SEPARATOR + '?[ \\t]+)?' + verbs(VERBS.bg.common) + NAME_SPACE + END_NAME
		]
	},
	hu: {
		end: [
			NAME_SPACE_OPTIONAL + SEPARATOR + NAME_SPACE_OPTIONAL + verbs(VERBS.hu.common) + NAME_SPACE + END_NAME
		],
	...
	pl: {
			...
			NAME_SPACE + SEPARATOR + NAME_SPACE + verbs(VERBS.pl.common) + NAME_SPACE + END_NAME
		],
		quoteMarks: [QUOTE_MARKS[3]],
		regs: [{
			reg: new RegExp('\\n- ' + QUOTE + NAME_SPACE + SEPARATOR + NAME_SPACE + verbs(VERBS.pl.common) + NAME_SPACE + END_NAME, 'gi'),
			name: 1,
			quote: 0
		}]
	},
	en: {
		...
		start: [
			...
			// – Shaun White
			NAME_SPACE_OPTIONAL + '[\n\r]+' + NAME_SPACE_OPTIONAL + SEPARATOR + NAME_SPACE_OPTIONAL + START_NAME + '(?:$|[\n\r])'
		]
	}
};
